...
plot.gradLasso <- function(x, which = c(1L:5L), ...) {
  plots <- list(); plot_names <- character(5)
  ...
  if (!is.null(x$boot_matrix)) {
    ...
    plots[[1]] <- function() {
      ...
      par(mar = c(7, 4, 4, 2))
      barplot(sel, main="Stability Selection (Top 30)", ylab="Selection Probability", 
  ...
  if (!is.null(x$residuals)) {
    y_hat <- x$fitted.values; residuals <- x$residuals
    ...
    plots[[3]] <- function() {
      ...
      try({ lines(lowess(y_hat, residuals), col="blue") }, silent=TRUE)
    }
...
residuals.gradLasso <- function(object, ...) {
  ...
  warning("Residuals not stored in object."); return(NULL)
}
...
logLik.gradLasso <- function(object, ...) {
  if (is.null(object$deviance)) return(NA)
