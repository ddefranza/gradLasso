...
get_lambda_seq <- function(X, y, family, n_lambda = 20, ratio = 0.001) {
  ...
  if (family$name == "zinb") dummy <- c(rep(0, p), 0)
  else if (family$name == "negbin") dummy <- c(rep(0, p), 0)
  else dummy <- rep(0, p)
...
cv.gradLasso <- function(object, data = NULL, family, lambdas = NULL, nfolds = 5, 
                         batch_size = NULL, subsample = NULL, parallel = FALSE) {
  ...
  if (!is.null(subsample) && subsample < n_total) {
    cat(sprintf("Subsampling CV: Using %d rows out of %d...\n", subsample, n_total))
